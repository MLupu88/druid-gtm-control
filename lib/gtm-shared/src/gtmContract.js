...
// Stronger wording used ONLY once isConfirmedPersistedDecision() proves the record was
// actually persisted — never selected from final_status alone.
...
// Confirmed persisted decision requires the backend to explicitly say so. No current
// n8n receiver response provides these fields, so this always returns false today.
...
// inferred from provider_status alone or from final_status="called". No current n8n
// receiver response provides these fields, so this always returns false today.
...
export function getTruthfulStatusPresentation(finalStatus, evidence) {
  if (isConfirmedExternalExecution(evidence)) {
    ...
    return {
      phase: "success",
      message: confirmedMessage || EXTERNAL_EXECUTION_CONFIRMED_FALLBACK,
    };
  }
  if (isConfirmedPersistedDecision(evidence)) {
    return {
      phase: "success",
      message:
        STATUS_CONFIRMED_LABELS_V3[finalStatus] ??
        STATUS_LABELS_V3[finalStatus] ??
...
// from final_status or arbitrary n8n text.
