...
export const BUTTONS = {
  ...
  approve_linkedin: { kind:"server", endpoint:"activate", body:{ channel:"linkedin" }, label:"Prepare LinkedIn message",           honest:"Prepares a self-serve LinkedIn message you copy or download and send yourself — this is not sent automatically, and no LinkedIn tool is connected." },
  ...
  promote_mql:      { kind:"ui", label:"Treat as Ready for Sales", honest:"Records a real MQL decision for this account so you can approve outreach — the same safety checks still apply on approval." },
  promote_mql_owner:{ kind:"ui", label:"Owner: promote to Ready for Sales", honest:"Records a real MQL decision, then notifies the account owner — same as above, plus the notification." },
  dismiss:          { kind:"ui", label:"Dismiss", honest:"Records a decision that removes this from your attention list. The account itself is never deleted — it stays visible in All accounts." },
...
export const VOICE_UNAVAILABLE_REASON = "Voice calling is unavailable pending compliance approval and provider setup. No call can be placed.";
...
export function buttonDisabled(buttonKey, row) {
  ...
  // Unconditional identified-contact requirement (product decision, 2026-07-24): a
  // prospect-facing draft must never be prepared for reconstructed_contact/company_level/
  // anonymous/unknown identity, regardless of any other gate. Checked first, before any
  // other condition, so no future bypass can be added ahead of it by accident.
  if ((buttonKey === "approve_email" || buttonKey === "approve_linkedin") && !hasIdentifiedContact(row)) {
    return { disabled: true, reason: IDENTIFIED_CONTACT_REQUIRED_REASON };
  }
...
export function buttonDisabledPhaseC(btn,row,cfg,previewOnly=false){
  ...
  // run before the previewOnly bypass immediately below, so Sample/preview mode can never
  // reveal a prospect-facing draft-preparation button for an insufficiently identified
  // account (reconstructed_contact/company_level/anonymous/unknown).
  if((btn==="approve_email"||btn==="approve_linkedin") && !hasIdentifiedContact(row)) return true;
  ...
  // Email-only legal-basis formula, unchanged from before this PR. LinkedIn deliberately
  // does NOT copy this formula — its own legal-basis policy is a separate, not-yet-defined
  // product/legal decision (see product decision, 2026-07-24). Do not add a LinkedIn
  // consent/region check here without an explicit product/legal sign-off.
...
// A prospect-facing Email/LinkedIn draft may be prepared ONLY for known_crm_contact or
// identified_contact. reconstructed_contact/company_level/anonymous/missing-or-unknown are
// NOT sufficient — no greeting, subject, body, copy, download, approval, or confirmation
// may ever be produced for those. Every layer (button visibility, disabled logic, the
// deterministic composer, the MessageComposer UI, and ActionModal confirmation) must call
// hasIdentifiedContact() rather than re-deriving this rule independently.
const SUFFICIENTLY_IDENTIFIED_KEYS = new Set(["known_crm_contact", "identified_contact"]);

// Shape-driven: account-queue rows carry identity_resolution; signal-queue rows carry
// resolution_level instead (never both — confirmed against QUEUE_COLUMNS and
// MOCK_ACCOUNT_QUEUE). Missing/unrecognized identity resolves to "anonymous", fail-closed,
// matching the existing conservative-default pattern used elsewhere (engine_mode,
// visitor_claim_allowed).
export function resolveIdentityKey(row) {
  if (row && Object.prototype.hasOwnProperty.call(row, "identity_resolution")) {
    return _lc(row.identity_resolution) || "anonymous";
  }
  const res = _lc(row?.resolution_level);
  if (res === "person") return "identified_contact";
  if (res === "company") return "company_level";
  return "anonymous";
}

export function hasIdentifiedContact(row) {
  return SUFFICIENTLY_IDENTIFIED_KEYS.has(resolveIdentityKey(row));
}

export const IDENTIFIED_CONTACT_REQUIRED_REASON =
  "An identified contact is required before preparing this message.";

// ===================== UNAVAILABILITY CATEGORIES (product decision, PR 2) ===============
// Distinguishes WHY an activation button is unavailable, so the UI never lumps a
// by-design state under a "coming later" framing that implies it will resolve on its own.
export const UNAVAILABILITY_CATEGORY = {
  VOICE_NOT_ENABLED: "voice_not_enabled",
  NOT_APPLICABLE: "not_applicable",
  TEMPORARY: "temporary",
};

export const PROSPECTING_NOT_APPLICABLE_REASON = "Not applicable for this recommendation.";

// Voice is unavailable until compliance approval and provider setup are complete (see
// VOICE_UNAVAILABLE_REASON). Email/LinkedIn are "not applicable" only when the row's
// output type itself is a NO_PROSPECT type (Pipeline Assist/Owner Alert/Retarget/
// Suppressed) — everything else (engine mode, consent/region) is a genuinely temporary,
// resolvable state and keeps its existing per-row reason text.
export function unavailabilityCategory(buttonKey, outputType) {
  if (buttonKey === "approve_call") return UNAVAILABILITY_CATEGORY.VOICE_NOT_ENABLED;
  if ((buttonKey === "approve_email" || buttonKey === "approve_linkedin") && NO_PROSPECT.has(outputType)) {
    return UNAVAILABILITY_CATEGORY.NOT_APPLICABLE;
  }
  return UNAVAILABILITY_CATEGORY.TEMPORARY;
}

// Whether approve_email/approve_linkedin are excluded from a row specifically because it
// lacks an identified contact — as opposed to the output type itself never offering them
// (e.g. Suppressed/Pipeline Assist never include them in buttonsForOutput regardless of
// identity). Lets the UI show IDENTIFIED_CONTACT_REQUIRED_REASON only when identity is
// truly the reason those actions are unavailable.
export function identityBlocksProspecting(row, outputType) {
  if (hasIdentifiedContact(row)) return false;
  const offered = buttonsForOutput(outputType);
  return offered.includes("approve_email") || offered.includes("approve_linkedin");
}
